...
function _make_configurations(vcprojfile, vsinfo, target, vcprojdir)
    ...
    vcprojfile:enter("<Configurations>")
        ...
        vcprojfile:enter("<Configuration")
            ...
            local relative_path = path.relative(path.absolute("$(plat)"), vcprojdir) --path.relative(path.absolute(config.get("buildir")), vcprojdir)          
			vcprojfile:print("OutputDirectory=\"%s\"", relative_path)

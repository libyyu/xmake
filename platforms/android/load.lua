...
function main()
    ...
    if arch:startswith("arm64") then
        ...
        _g.shflags      = {"-llog"}
        _g.cxxflags     = {}
    elseif arch:startswith("x86") then
        _g.cxflags      = {}
        _g.asflags      = {}
        _g.ldflags      = {"-llog"}
        _g.shflags      = {"-llog"}
    ...
    if ndk and ndk_sdkver then
        ...
        elseif arch:startswith("x86") then
            insert(_g.cxflags, format("--sysroot=%s/arch-x86", ndk_sdkdir))
            insert(_g.asflags, format("--sysroot=%s/arch-x86", ndk_sdkdir))
            insert(_g.ldflags, format("--sysroot=%s/arch-x86", ndk_sdkdir))
            insert(_g.shflags, format("--sysroot=%s/arch-x86", ndk_sdkdir))
        else
